...
function likedislikePost(postID, checker) {
  fetch('/ld-post', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({ postID: postID, checker: checker })
  })
    .then(response => response.json())
    .then(data => {
      if (data.success) {
      ...
      if (data.success) {
        ...
        posts.forEach(post => {
          ...
          newPost.innerHTML = '<h2 class="blog-post-title">'
            ...
            '</a></p><p>'
            + post.Content +
            '</p><hr><div class="buttons"><button class="like-dislike-btn" onclick="likedislikePost(\'' + post.PostID, true, + '\')"><img src="../png/like.png" alt="Like Icon">Like <span class="like-count">0</span></button><button class="like-dislike-btn" onclick="likedislikePost(\'' + post.PostID, false, + '\')"><img src="../png/dislike.png" alt="Dislike Icon">Dislike <span class="dislike-count">0</span></button><button class="reply-btn" onclick="writeComment(this)">Comment</button>'
